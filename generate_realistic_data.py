...
def generate_realistic_training_data():
    ...
    # Clear old training tokens
    Token.objects.filter(status='Completed').delete()
    ...
    for day in range(60):
        ...
        for doctor in doctors:
            ...
            for i in range(num_patients):
                ...
                # Appointment scheduled every 15-20 minutes
                appointment_time = clinic_start + timedelta(minutes=i * random.randint(15, 20))
                ...
                # Determine category
                if wait_minutes <= 10:
                    predicted_category = 'Short'
                elif wait_minutes <= 20:
                    predicted_category = 'Medium'
                else:
                    predicted_category = 'Long'
                
                # Create token
                Token.objects.create(
                    patient=patient,
                    doctor=doctor,
                    token_number=f"T{day:03d}{i:02d}",
                    status='Completed',
                    appointment_time=appointment_time,
                    consultation_start_time=consultation_start,
                    completed_at=completed_at,
                    predicted_wait_time=wait_minutes,
                    predicted_wait_category=predicted_category,
    ...
    # Show distribution
    short = Token.objects.filter(status='Completed', predicted_wait_category='Short').count()
    medium = Token.objects.filter(status='Completed', predicted_wait_category='Medium').count()
    long = Token.objects.filter(status='Completed', predicted_wait_category='Long').count()
